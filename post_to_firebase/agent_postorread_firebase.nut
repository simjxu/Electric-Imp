...
const FIREBASE_NAME = "ENTER FIREBASE NAME"
const FIREBASE_AUTH_KEY = "ENTER AUTH KEY"

firebase <- Firebase(FIREBASE_NAME, null);

function func(data_received){
    
    // This reads from the database
    server.log("read from database");
    firebase.read("/newkey/newnested_key", function(data) {
        server.log("data: " + data);
    });
    
    // This writes to database
    server.log("write to database");
    firebase.write("/newkey/newnested_key", data_received);

    server.log("function complete");
}
